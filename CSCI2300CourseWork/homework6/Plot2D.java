...
{ 
    ...
    private ArrayList<GeneralPath> storedPlots;
    ...
    public Plot2D()
    {
       ...
       //Initializes the storedPlots array
       storedPlots = new ArrayList<GeneralPath>();
    }
    ...
    public void plot(Function2D f)
    {
	   ...
	   storedPlots.add(new GeneralPath(GeneralPath.WIND_EVEN_ODD, numPoints));
       storedPlots.get(0).moveTo(scale*0, scale*(maxY-f.eval(minX)));
       ...
       while (current <= maxX)
       {
          ...
          storedPlots.get(0).lineTo(scale*(current-minX), scale*(maxY-yValue));
       }
       // re-draw the picture
       this.repaint();
    ...
    public void paint(Graphics g)
    {
	   ...
	   // Draws all the plots stored in the storedPlot arraylist
       for(int i = 0; i <storedPlots.size(); i++)
       {
		   g2.draw(storedPlots.get(i));
	   }
    }
